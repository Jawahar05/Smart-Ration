...
<body class="body">
        ...
        <nav class="navbar navbar-expand-sm navbar-light bg-light shadow fixed-top justify-content-between">
            ...
            <div>
                ...
                <ul class="navbar-nav nav-color">
                    <li>
                        <a class="navbar-brand" href="../admin/stocks&stores.php"><i class="fas fa-shopping-cart" style="color:#0097e6;"></i>
                            Allocate</a>
                    </li>
                </ul>
            </div>
    ...
    <div class="padding container">
        ...
        $dis =  preg_replace("/\s+/", "", $_SESSION['storedist']);
        $getdata = "SELECT *FROM district_data WHERE district_name = '$dis'";
        $execute = mysqli_query($conn, $getdata);
        if (mysqli_num_rows($execute) == 1) {
            while ($row = mysqli_fetch_assoc($execute)) {
                $noofshops = $row['no_of_shops'];
        ...
        <div class="row text-primary" style="font-weight:bolder;">
            <div class="col-sm">
                <p class="text-right" for="district">District :</p>
            </div>
            <div class="col-sm" name="district">
                <p><?php echo ($dis); ?></p>
            </div>
            <div class="col-sm">
                <p class="text-right" for="stores">Total Stores :</p>
            </div>
            <div class="col-sm">
                <p><?php echo ($noofshops); ?></p>
            </div>
        </div>
